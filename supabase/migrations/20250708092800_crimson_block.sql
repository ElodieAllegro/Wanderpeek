/*
  ...
  1. New Tables
    ...
    - `hotels` - Hotel listings with approval status
  ...
  2. Security
    ...
    - Secure admin, hotelier, and client data access

  3. Sample Data
    - Admin user: adminwonderpick@gmail.com
    - Hoteliers: hotelPark@gmail.com, hotelOcean@gmail.com
    - Test clients and sample hotels/activities
*/
...
SELECT 
  h.id,
  generate_series(CURRENT_DATE, CURRENT_DATE + INTERVAL '2 months', '1 day'::interval)::date,
  CASE WHEN random() > 0.1 THEN true ELSE false END, -- 90% availability
  CASE WHEN EXTRACT(dow FROM generate_series(CURRENT_DATE, CURRENT_DATE + INTERVAL '2 months', '1 day'::interval)) IN (0,6) 
       THEN h.price_per_night * 1.2 
       ELSE null END -- Weekend premium
FROM hotels h
